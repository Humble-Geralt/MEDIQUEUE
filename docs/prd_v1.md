...
本文档基于 [native_description.md](./native_description.md) 中的原始反馈，重新建模医院智能叫号系统的业务背景、角色诉求、流程规则与产品范围，形成一份可指导设计、开发、测试和验收的正式 PRD。
...
| ~~就诊单 Visit~~ | ~~一次具体门诊就诊任务~~ | ~~就诊单ID、科室、诊室、预约时间、就诊类型、关联排队票号~~ |
| 排队票号 QueueTicket | 一次排队凭证与排序单元 | 票号、当前序位、状态、优先级、签到时间、**关联患者** |
| 紧急优先申请 PriorityRequest | 患者发起的插队/优先请求 | 申请ID、描述文本、AI结果、人工审核结果、~~理由~~ |
| 叫号事件 CallEvent | 一次实际叫号动作 | 事件ID、票号、诊室、发起人、时间、播报状态 |
| 队列快照 QueueSnapshot | 某时刻完整队列状态 | 当前叫号、待叫列表、暂停状态、版本号（时间戳） |
| ~~终端设备 Terminal~~ | ~~系统访问入口~~ | ~~设备ID、终端类型、在线状态、最后同步时间~~ |
| ~~审计记录 AuditLog~~ | ~~关键动作留痕~~ | ~~操作人、动作、前后状态、时间、备注~~ | 
> review意见：出于时间限制考虑，砍掉了非必要的业务对象模型。就诊单相关融入排队票号内，移除审计和终端设备。优先申请移除理由（跟描述文本重复）
...
1. 紧急优先申请不等于自动插队。患者提交后，AI 先做紧急度评估，~~再由护士确认是否前移~~ **直接弹出AI审核意见和患者原始输入到医生web pc端，医生来做决策批准 or 不批准**。
2. 非医疗理由不能构成优先依据。如“我是 VIP”“我赶时间”等必须被识别为无效优先理由。
3. ~~所有队列调整必须留痕，且对护士端可见，对患者端要可解释。~~ **没有护士端**
4. 大屏展示以号码为主、姓名为辅，优先保证识别效率；~~是否全名展示可由院方配置~~ **中文名默认替换第二个字为 X 并按性别展示，例如 李X伟 先生、刘X菲 女士**。
5. 医生只能操作与自己诊室相关的叫号，不应直接修改全局优先级规则。
6. ~~系统暂停时，不得继续自动推进下一位，但允许查看当前队列和关键状态。~~ **不考虑系统整体暂停**
7. ~~断网期间必须允许继续最低限度叫号，但不能产生无法追溯的黑盒操作。~~ **断网期间各个终端鲜明的显示目前的断网状态以及断网前最新的队列状态，循环公告（双语）告知患者系统网络故障，不要紧张，听从当前在场工作人员或护士的指令**

### 5.5 核心业务流程

```mermaid
flowchart LR
    A["患者取号/进入队列"] --> B["患者端查看排队状态"]
    B --> C["等待叫号"]
    C --> D["医生端发起叫号"]
    D --> E["大屏高亮 + 双语播报"]
    D --> F["患者端提醒"]
...
> 原始需求未强制要求单独实现护士端界面，但从真实业务看，它是闭环成立的关键角色，因此在 PRD 中纳入 MVP 需求。
...
| 功能 | 说明 | 理由 | 人类评审意见 |
| --- | --- | --- | --- |
| 队列实时同步 | 医生端、大屏端、患者端状态一致 | 是整个叫号系统成立的基础 | 保留为 P0 |
| 医生端叫号/跳过/暂停 | 医生可推进队列 | 是主业务闭环的核心操作 | 保留为 P0 |
| 患者端个人排队信息 | 展示票号、前方人数、预计等待时间 | 直接缓解焦虑与重复询问 | 保留为 P0 |
| 候诊大屏高可读展示 | 大字号、高对比、当前叫号高亮 | 直接解决老年患者识别问题 | 保留为 P0 |
| 中英文界面与播报 | 支持外籍患者理解 | 原始反馈明确提出 | 界面和播报应当拆开，界面保留为P0，播报实际是P2 |
| 紧急优先申请 | 患者可提交症状理由 | 支撑特殊患者优先机制 | 保留为 P0 |
| ~~AI 结构化评估结果~~ | ~~为护士提供辅助判断~~ | ~~降低争议与主观性~~ | 本质上属于“紧急优先申请”的辅助能力，不单独作为 P0 功能；如需保留，应降级到 P1 |
| ~~护士~~医生人工审核优先申请 | 人工掌握最终决策 | 医疗安全底线要求 | 审核主体应调整为医生；保留为 P0 |
| 断网检测与离线降级方案 | 网络异常时持续可用 | 医疗安全高风险点 | 从 P0 降级到 P1 |
| ~~审计日志~~ | ~~记录插队与关键操作~~ | ~~解决“为什么插队”的可解释性~~ | 暂时不考虑，移除 |

### 10.2 P1（重要但可延后）

| 功能 | 说明 | 理由 | 人类评审意见 |
| --- | --- | --- | --- |
| 过号重返队列策略 | 患者返回后按规则恢复位置 | 高频真实问题，但可次阶段细化 | 保留为 P1 |
| ~~诊室地理引导~~ | ~~患者端显示位置/路线~~ | ~~提升体验，但不影响核心叫号~~ | 不考虑，移除；患者端只显示诊室号码即可 |
| ~~多科室统一队列看板~~ | ~~跨诊区管理~~ | ~~MVP 可先单诊室/单科室~~ | 暂不作为当前阶段能力 |
| 更细粒度的隐私模式 | 姓名掩码、展示策略可配 | 重要但不阻塞最小闭环 | 从 P1 降级到 P2 |
| ~~统计报表~~ | ~~等待时长、过号率、争议率~~ | ~~对管理有价值，但不影响 MVP 运行~~ | 暂时不考虑，移除 |
...
| 功能 | 说明 | 理由 | 人类评审意见 |
| --- | --- | --- | --- |
| ~~更多语种支持~~ | ~~如日语、韩语等~~ | ~~当前原始需求只明确英文~~ | 当前业务场景只明确提出英语，不考虑 |
| 预计等待时间智能预测 | 根据历史节奏动态估算 | 复杂度高，MVP 先用规则估算 | 保留为 P2 |
| ~~现场硬件联动~~ | ~~灯牌、打印叫号票、短信提醒~~ | ~~增强体验，但非当前主线~~ | 当前不考虑，移除 |
| 患者满意度收集 | 叫号后反馈 | 对运营有益，但优先级较低 | 保留为 P2 |
